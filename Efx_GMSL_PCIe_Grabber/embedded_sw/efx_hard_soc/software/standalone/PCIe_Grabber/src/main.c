...
#ifdef STATIC_IMAGE
/// @brief Move image data from NOR flash to memory, and extract them from
/// LZ4. Assign the proper address for 1 single image and its resized planar image address
void init_test_images(void) {
	bsp_printf("Moving images LZ4 from FLASH ADDR: 0x%x to  RAM ADDR: 0x%x, size = %d KiB\r\n",
			IMAGES_FLASH_ADDR, IMAGES_LZ4_BASE, SIZE_KiB(IMAGES_LZ4_SIZE));
	moveFlash2Mem(SPI0, 0, IMAGES_FLASH_ADDR, IMAGES_LZ4_BASE,
			IMAGES_LZ4_SIZE);

	int ret = decompressLZ4((void *)IMAGES_LZ4_BASE, IMAGES_LZ4_SIZE,
			(void *)IMAGES_RGB_BASE, IMAGES_RGB_TOTAL_SIZE);
	for (size_t i = 0; i < SAMPLE_IMAGES_N; ++i) {
		sample_images[i] = IMAGES_RGB_BASE + IMAGES_RGB_SIZE * i;
		bsp_printf("sample_images[%d] = 0x%x\r\n", i, sample_images[i]);

#ifdef SW_PLANAR
		planar_images[i] = PLANAR_RGB_BASE + (MODEL_INPUT_SIZE) * i;
		bsp_printf("planar_images[%d] = 0x%x\r\n\n", i, planar_images[i]);
#endif
	}
}
#endif
...
#ifdef SW_PLANAR
/// @brief Process input 1080x1920 RGBX8 images into 384x384x3 planar RGB images
/// for CNN input.
///
/// Does in sequence:
///
///  - Bilinear downscaling 1920x1080 to 384x216
///
///    - Integer factor 5 makes it equivalent to nearest-neighbour
///
///  - Letterboxing
///
///  - Packed-to-planar
void downscalePlanarRGB(const size_t n, const size_t src_h, const size_t src_w,
		const size_t dst_h, const size_t dst_w, bool verbose,
		volatile uint32_t *src_base,
		volatile uint8_t *dst_base) {
	uint64_t timecmp_0, timecmp_1;
	timecmp_0 = clint_getTime(BSP_CLINT);

	int log_count1 = 1, log_count2 = 2, log_count3 = 2, log_count4 = 2;

	// bsp_print("Before float \r\n");
	float ratio_h = (float)src_h / (float)dst_h;
	float ratio_w = (float)src_w / (float)dst_w;
	float ratio = (ratio_h > ratio_w) ? ratio_h : ratio_w;
	if (verbose)
		bsp_printf("ratio_h = %f, ratio_w = %f, ratio = %f \r\n", ratio_h, ratio_w,
				ratio);

	unsigned letterbox_h = 0, letterbox_w = 0;
	if (!fequals(ratio_h, ratio_w)) {
		if (ratio_h > ratio_w) {
			letterbox_w = (dst_w - (unsigned)round(src_w / ratio)) >> 1;
		} else {
			letterbox_h = (dst_h - (unsigned)round(src_h / ratio)) >> 1;
		}
	}
	if (verbose)
		bsp_printf("letterbox_h = %d, letterbox_w = %d \r\n", letterbox_h,
				letterbox_w);

	float y, x;
	unsigned y_floor, y_ceil, x_floor, x_ceil;
	bool y_exact, x_exact;
	uint32_t p1, p2, p3, p4;
	uint8_t c1, c2, c3, c4, c;
	float q1, q2, q;
	size_t offset, ii, jj;
	size_t src_frame_size = src_h * src_w;
	size_t dst_frame_size = dst_h * dst_w;

	for (size_t nn = 0; nn < n; ++nn) {
		for (size_t hh = 0; hh < dst_h; ++hh) {
			if (letterbox_h != 0 && (hh < letterbox_h || hh + letterbox_h >= dst_h)) {
				for (size_t ww = 0; ww < dst_w; ++ww) {
					for (size_t cc = 0; cc < 3; ++cc) {
						offset = dst_frame_size * (3 * nn + cc) + dst_w * hh + ww;
						dst_base[offset] = 114;
					}
				}
			} else {
				for (size_t ww = 0; ww < dst_w; ++ww) {
					ii = (letterbox_h == 0) ? hh : hh - letterbox_h;
					jj = (letterbox_w == 0) ? ww : ww - letterbox_w;
					y = ratio * ii;
					x = ratio * jj;
					y_floor = floor(y);
					x_floor = floor(x);
					y_ceil = fmin(ceil(y), src_h - 1);
					x_ceil = fmin(ceil(x), src_w - 1);
					y_exact = y_floor == y_ceil;
					x_exact = x_floor == x_ceil;
					p1 = src_base[src_frame_size * nn + y_floor * src_w + x_floor];
					p2 = src_base[src_frame_size * nn + y_floor * src_w + x_ceil];
					p3 = src_base[src_frame_size * nn + y_ceil * src_w + x_floor];
					p4 = src_base[src_frame_size * nn + y_ceil * src_w + x_ceil];
					for (size_t cc = 0; cc < 3; ++cc) {
						offset = dst_frame_size * (3 * nn + cc) + dst_w * hh + ww;
						if (letterbox_w != 0 &&
								(ww < letterbox_w || ww + letterbox_w >= dst_w)) {
							dst_base[offset] = 114;
						} else {
							c1 = (p1 >> (8 * cc)) & 0xFF;
							c2 = (p2 >> (8 * cc)) & 0xFF;
							...
							if (y_exact && x_exact) {
								c = c1;
								// LOG_PREPROCESS(log_count1-- > 0);
							} else if (y_exact) {
								q = (x - x_floor) * c1 + (x_ceil - x) * c2;
								c = round(q);
								// LOG_PREPROCESS(log_count2-- > 0);
							} else if (x_exact) {
								q = (y - y_floor) * c1 + (y_ceil - y) * c3;
								c = round(q);
								// LOG_PREPROCESS(log_count3-- > 0);
							} else {
								q1 = (x - x_floor) * c1 + (x_ceil - x) * c2;
								q2 = (x - x_floor) * c3 + (x_ceil - x) * c4;
								q = (y - y_floor) * q1 + (y_ceil - y) * q2;
								c = round(q);
								// LOG_PREPROCESS(log_count4-- > 0);
							}
							dst_base[offset] = c;
						}
					}
	...
	timecmp_1 = clint_getTime(BSP_CLINT);
	printPTime(timecmp_0, timecmp_1,
			"Done sw downscale & packed-to-planar, took clock cycles:");
}
#endif
...
//
	recv_dma(DMASG_CAM_S2MM_CHANNEL, DMASG_CAM_S2MM_PORT, buf_offset(camera_buffer, IMAGE_START_OFFSET), FRAME_WIDTH*FRAME_HEIGHT*4, 1, 0, 1);
