...
(
  ...
  // CSI controller ouptut interface port
	...
	output 	[NUM_CHANNEL*4-1:0]					  rx_vsync_pulse
		...
		(
			...
			begin:csi_rx_monitor
				...
				video_monitor inst_csi_rx_monitor(
					...
					.ref_clk_hz(32'd222_222_222), // Frequency of the pixel clock (for rate calc)
                    .in_pixel_line_num (w_pixel_line_num ),
                    .in_pixel_frame_num(w_pixel_frame_num),
					
					.hsync_per_frame(w_rx_hsync_per_frame),
					.pixel_per_line (w_rx_pixel_per_line ),
					.frame_rate     (w_rx_frame_rate     ), // Measured in Hz
					
					.vsync_pulse    (rx_vsync_pulse[x*4 +y])
