...
1. **Prepare the daughter cards**  
   - Connect the two Raspberry Pi camera modules to both top connector (FPC2) and bottom connector (FPC1) of Dual Pi Camera Connector Board using a 15‑pin FFC.
   - Duplicate to obtain 2 sets of Dual Pi Camera Connector Boards, connecting a total of 4 camera modules. *(each development kit contains 2 cameras)*

2. **Insert the daughter cards onto the Ti180J484 Development Board**  
   - Attach the daughter cards onto QSE connectors **P1** and **P2** on the Ti180J484 development board.  

3. **Connect the ADI GMSL Serializer EVK**  
   - Use 22-pin FFC Cable (Type A) to connect IMX477 Camera Connector Daughter Card (**FPC1**, bottom connector) with the ADI GMSL Serializer EVK (Adapter: **P9**) and insert it onto the Ti180J484 Development Board **P3**.  
...
4. **Configure jumper settings**  
   ...
   <img src="images/gmsl_4cam_sh_final-hw-setup.png" alt="Sensor Hub Hardware Setup" width="800">  
...
5. **FPGA Configuration**  
   ...
   - Choose the efx-gmsl-sensorhub-line-4x1080p_combined.hex in prebuild/efx-gmsl-video-line-4x1080p/quickstart folder (from release build)
