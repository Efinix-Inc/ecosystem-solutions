...
module csi_tx_controllers_full #(
...
(
  ...
  // CSI controller input interface port
	...
	input  [NUM_CHANNEL*64-1:0] tx_in_data,
    input  [NUM_CHANNEL*16-1:0] tx_in_vs,
    input  [NUM_CHANNEL*16-1:0] tx_in_hs, 
	
	input [NUM_CHANNEL*6-1:0]  tx_in_dt,   // data type of the Long Packet
    input [NUM_CHANNEL*16-1:0] tx_in_haddr,   //16 bit total Horizontal number of pixels
	
	input [NUM_CHANNEL*16-1:0] tx_in_line_num,    //
    input [NUM_CHANNEL*16-1:0] tx_in_frame_num,   //
    
    
    output [NUM_CHANNEL-1:0] tx_fifo_full,
    output [NUM_CHANNEL-1:0] tx_fifo_empty
...
generate 
	...
	begin:csi_tx
		...
		wire	w_video_valid;
		wire	w_video_de;
		wire	[15:0]w_video_hs;
		wire	[15:0]w_video_vs;
        
        
		wire	[63:0] w_video_data;
        wire [31:0] w_mipi_debug_in;
        ...
        assign w_fifo_full  = w_mipi_debug_out[0];
        assign w_fifo_empty = w_mipi_debug_out[1];
        
        assign tx_fifo_full [x] = w_mipi_debug_out[0];
        assign tx_fifo_empty[x] =  w_mipi_debug_out[1];   
    ...
    /*   efx_csi2_tx_top_tx //efx_csi2_tx_top_tx
		#(
            ...
            .NUM_DATA_LANE       (NUM_TX_PER_CHANNEL),
            .PACK_TYPE           (4'b1111),
            .PIXEL_FIFO_DEPTH    (4096),  //set this to 2 power value of DEPTH = (MAX_HRES/8) x 2
            .ENABLE_VCX          (0),
            .FRAME_MODE          ("GENERIC"),    //1-ACCURATE, 0-GENERIC
            .ENABLE_SKEWCAL_INIT (1),
            .ASYNC_STAGE         (2)
		)inst_efx_csi2_tx */
		csi2_hard_mipi_tx inst_efx_csi2_tx
		(
		//	.reset_n			(w_rstn),
		//	.clk				(clk),
			...
			.hsync_vc0			(w_video_hs[0]),
			.hsync_vc1			(w_video_hs[1]),
			.hsync_vc2			(w_video_hs[2]),
			.hsync_vc3			(w_video_hs[3]),
			.hsync_vc4			(w_video_hs[4]),
			.hsync_vc5			(w_video_hs[5]),
			.hsync_vc6			(w_video_hs[6]),
			.hsync_vc7			(w_video_hs[7]),
			.hsync_vc8			(w_video_hs[8]),
			.hsync_vc9			(w_video_hs[9]),
			.hsync_vc10			(w_video_hs[10]),
			.hsync_vc11			(w_video_hs[11]),
			.hsync_vc12			(w_video_hs[12]),
			.hsync_vc13			(w_video_hs[13]),
			.hsync_vc14			(w_video_hs[14]),
			.hsync_vc15			(w_video_hs[15]),

			.vsync_vc0			(w_video_vs[0]),
			.vsync_vc1			(w_video_vs[1]),
			.vsync_vc2			(w_video_vs[2]),
			.vsync_vc3			(w_video_vs[3]),
			.vsync_vc4			(w_video_vs[4]),
			.vsync_vc5			(w_video_vs[5]),
			.vsync_vc6			(w_video_vs[6]),
			.vsync_vc7			(w_video_vs[7]),
			.vsync_vc8			(w_video_vs[8]),
			.vsync_vc9			(w_video_vs[9]),
			.vsync_vc10			(w_video_vs[10]),
			.vsync_vc11			(w_video_vs[11]),
			.vsync_vc12			(w_video_vs[12]),
			.vsync_vc13			(w_video_vs[13]),
			.vsync_vc14			(w_video_vs[14]),
			.vsync_vc15			(w_video_vs[15]),
			...
			.haddr              (tx_in_haddr),  		
			.line_num			(tx_in_line_num),
			.frame_num			(tx_in_frame_num),	
			.irq				(),
            
            .mipi_debug_in      (w_mipi_debug_in),
            .mipi_debug_out     (w_mipi_debug_out)
            
		);
    
		assign w_video_valid = tx_in_de[x];
		assign w_video_vs	 = tx_in_vs[16*x +: 16];
		assign w_video_hs	 = tx_in_hs[16*x +: 16];
		
		assign w_video_data  = tx_in_data[64*x +: 64];
	end 
