...
localparam T1W = 8;
`ifdef EFX_SIM
localparam SRCADDR = 32'h00010000;
localparam DSTADDR = 32'h00020000;
localparam FRAME = 5;
localparam BUFFER_SIZE = 1280*4;
`else
localparam SRCADDR = 32'h00010000;
localparam DSTADDR = 32'h04010000;
localparam FRAME = 16;
localparam BUFFER_SIZE = 1024*1024*4;
`endif
//Reset and PLL
wire 		              io_masterReset;
wire		              io_systemReset;
...
wire   [3:0]          io_ddrA_arw_payload_qos;
wire   [2:0]          io_ddrA_arw_payload_prot;
wire   [3:0]          io_ddrA_arw_payload_cache;
wire   [3:0]          io_ddrA_arw_payload_region;
...
wire 	 [1:0]	        tester_0_tvalid;	
wire   [1:0]	        tester_0_tready;	
wire   [2*T0W-1:0]	  tester_0_tdata;
wire   [2*T0W/8-1:0]	tester_0_tkeep;	
wire   [7:0]	        tester_0_tdest;	
wire   [1:0]          tester_0_tlast;	
wire 	 [1:0]	        tester_1_tvalid;	
wire   [1:0]	        tester_1_tready;	
wire   [2*T1W-1:0]	  tester_1_tdata;
wire   [2*T1W/8-1:0]	tester_1_tkeep;	
wire   [7:0]	        tester_1_tdest;	
wire   [1:0]          tester_1_tlast;	
wire   [1:0]          sg_cmd_channelId;
wire                  sg_cmd_valid;
wire                  sg_cmd_ready;
...
wire                  sg_cmd_write;
wire                  sg_rsp_valid;
wire   [1:0]          sg_rsp_channelId;
wire   [31:0]         sg_rsp_srcAddress;
wire   [31:0]         sg_rsp_dstAddress;
wire   [25:0]         sg_rsp_bytes;
wire                  sg_rsp_last;
wire                  sg_rsp_stall;
wire   [3:0]          ctrl_interrupts;
/////////////////////////////////////////////////////////////////////////////
//Clock and reset

assign io_masterReset 		      = ~( buttonReset & sys_pll_locked & peri_pll_locked & ddr_pll_locked );
assign io_ddrA_b_payload_resp 	= 2'b00;

ddr_reset_sequencer#(
  .FREQ(100)
) reset_sequencer_inst (
...
dma_loopback_tester #(
	...
	.tester_clk		        (io_peripheralClk               ),
	.tester_resetn		    (~io_peripheralReset            ),
 	.tester_i_tvalid	    (tester_0_tvalid[0]             ),	
 	.tester_i_tready	    (tester_0_tready[0]             ),	
 	.tester_i_tdata		    (tester_0_tdata[T0W-1:0]        ),	
 	.tester_i_tkeep		    (tester_0_tkeep[T0W/8-1:0]	    ),	
 	.tester_i_tdest		    (tester_0_tdest[3:0]	          ),	
 	.tester_i_tlast		    (tester_0_tlast[0]	            ),	
 	.tester_o_tvalid	    (tester_0_tvalid[1]             ),	
 	.tester_o_tready	    (tester_0_tready[1]             ),
 	.tester_o_tdata		    (tester_0_tdata[T0W*2-1:T0W]	  ),	
 	.tester_o_tkeep		    (tester_0_tkeep[T0W/4-1:T0W/8]  ),	
 	.tester_o_tdest		    (tester_0_tdest[7:4]	          ),	
 	.tester_o_tlast		    (tester_0_tlast[1]	            )	
);
...
dma_loopback_tester #(
	...
	.tester_clk		        (io_memoryClk                   ),
	.tester_resetn		    (~io_memoryReset                ),
 	.tester_i_tvalid	    (tester_1_tvalid[0]             ),	
 	.tester_i_tready	    (tester_1_tready[0]             ),	
 	.tester_i_tdata		    (tester_1_tdata[T1W-1:0]        ),	
 	.tester_i_tkeep		    (tester_1_tkeep[T1W/8-1:0]	    ),	
 	.tester_i_tdest		    (tester_1_tdest[3:0]	          ),	
 	.tester_i_tlast		    (tester_1_tlast[0]	            ),	
 	.tester_o_tvalid	    (tester_1_tvalid[1]             ),	
 	.tester_o_tready	    (tester_1_tready[1]             ),
 	.tester_o_tdata		    (tester_1_tdata[T1W*2-1:T1W]	  ),	
 	.tester_o_tkeep		    (tester_1_tkeep[T1W/4-1:T1W/8]  ),	
 	.tester_o_tdest		    (tester_1_tdest[7:4]	          ),	
 	.tester_o_tlast		    (tester_1_tlast[1]	            )	
);
...
dma_inst0 dma_inst(
  ...
  .ctrl_PADDR			      ({2'b00,io_apbSlave_0_PADDR}	          ),
  ...
  .dat0_o_clk			      (io_peripheralClk		            ),
  .dat0_o_reset		      (io_peripheralReset		          ),
  .dat0_o_tvalid		    (tester_0_tvalid[0]	            ),
  .dat0_o_tready		    (tester_0_tready[0]	            ),
  .dat0_o_tdata		      (tester_0_tdata[T0W-1:0]		    ),
  .dat0_o_tkeep		      (tester_0_tkeep[T0W/8-1:0]		  ),
  .dat0_o_tdest		      (tester_0_tdest[3:0]		        ),
  .dat0_o_tlast		      (tester_0_tlast[0]		          ),
  .io_0_descriptorUpdate(io_0_descriptorUpdate          ),
  .dat1_i_clk			      (io_peripheralClk		            ),
  .dat1_i_reset		      (io_peripheralReset	            ),
  .dat1_i_tvalid		    (tester_0_tvalid[1]	            ),
  .dat1_i_tready		    (tester_0_tready[1]	            ),
  .dat1_i_tdata		      (tester_0_tdata[T0W*2-1:T0W]    ),
  .dat1_i_tkeep		      (tester_0_tkeep[T0W/4-1:T0W/8]	),
  .dat1_i_tdest		      (tester_0_tdest[7:4]		        ),
  .dat1_i_tlast		      (tester_0_tlast[1]		          ),
  .io_1_descriptorUpdate(io_1_descriptorUpdate          ),
  .dat2_o_clk			      (io_memoryClk		                ),
  .dat2_o_reset		      (io_memoryReset		              ),
  .dat2_o_tvalid		    (tester_1_tvalid[0]	            ),
  .dat2_o_tready		    (tester_1_tready[0]	            ),
  .dat2_o_tdata		      (tester_1_tdata[T1W-1:0]		    ),
  .dat2_o_tkeep		      (tester_1_tkeep[T1W/8-1:0]		  ),
  .dat2_o_tdest		      (tester_1_tdest[3:0]		        ),
  .dat2_o_tlast		      (tester_1_tlast[0]		          ),
  .dat3_i_clk			      (io_memoryClk		                ),
  .dat3_i_reset		      (io_memoryReset	                ),
  .dat3_i_tvalid		    (tester_1_tvalid[1]	            ),
  .dat3_i_tready		    (tester_1_tready[1]	            ),
  .dat3_i_tdata		      (tester_1_tdata[T1W*2-1:T1W]    ),
  .dat3_i_tkeep		      (tester_1_tkeep[T1W/4-1:T1W/8]	),
  .dat3_i_tdest		      (tester_1_tdest[7:4]		        ),
  .dat3_i_tlast		      (tester_1_tlast[1]		          ),
...
(
  ...
  .userInterruptA                     (ctrl_interrupts[0]                ),
  .userInterruptB                     (ctrl_interrupts[1]                ),
  .userInterruptC                     (ctrl_interrupts[2]                ),
  .userInterruptD                     (ctrl_interrupts[3]                ), 
