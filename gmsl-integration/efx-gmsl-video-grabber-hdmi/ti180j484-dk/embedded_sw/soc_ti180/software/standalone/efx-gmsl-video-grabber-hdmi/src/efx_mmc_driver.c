...
int sd_ctrl_cmd(struct mmc *mmc, struct mmc_cmd *cmd)
{
	int time_out;
	u32 Value;
...
int sd_ctrl_creat_Descriptor(struct mmc *mmc,u32 blocks,u32 block_size ,char* src)
{
	u32 Value=0;
	u32 addr_location;
...
int sd_ctrl_data(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
	u32 buf=0,tmp=0;
	struct sd_ctrl_dev *dev =mmc->priv;
...
int sd_ctrl_check_read_write(struct mmc_cmd *cmd)
{
...
int sd_ctrl_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
