...
volatile uint8_t ecnn_intc_done=0;
...
void userInterrupt(){
    ...
    while(claim = plic_claim(BSP_PLIC, BSP_PLIC_CPU_0)){
    	...
    	case SYSTEM_PLIC_USER_INTERRUPT_E_INTERRUPT: //DMA channels share single interrupt
    		if ( cam_s2mm_active &&  dmasg_read_channelState(DMASG_BASE, DMASG_CAM_S2MM_CHANNEL, DMASG_CHANNEL_INTERRUPT_INPUT_PACKET_MASK) )
    		{
    			dmasg_interrupt_pending_clear(DMASG_BASE, DMASG_CAM_S2MM_CHANNEL, DMASG_CHANNEL_INTERRUPT_INPUT_PACKET_MASK);  //Disable dmasg channel interrupt
...
void IntcInitialize()
{
	...
	//Fix floating-point crash by enabling MSTATUS_FS, on top of the original:
//	csr_write(mstatus, csr_read(mstatus) | MSTATUS_MPP | MSTATUS_MIE);
	csr_write(mstatus, csr_read(mstatus) | MSTATUS_FS | MSTATUS_MPP | MSTATUS_MIE);
}
