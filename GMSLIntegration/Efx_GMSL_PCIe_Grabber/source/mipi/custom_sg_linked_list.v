`define APB3_SLV_REG_IN_CHANNEL 'h100
...
apb3_dma_sg_slave_oob #(
    ...
    .sg_current_vc(sg_out_control[7:0])
	...
	begin
		if(!resetn)
			...
			begin
				...
				`APB3_SLV_REG_IN_CHANNEL  : slaveRegOut <={24'd0, sg_current_vc[7:0]};
				default: 
				begin 
                    slaveRegOut <= slaveRegOut;
					//slaveRegOut <= slaveReg[PADDR[ADDR_WIDTH-1:2]];
				end
				endcase
			end 
			else
