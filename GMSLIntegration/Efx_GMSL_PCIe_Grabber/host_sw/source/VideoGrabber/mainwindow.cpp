...
static const std::string INTR_NAME = "edma";
...
InterruptType check_InterruptType()
{
    std::ifstream proc("/proc/interrupts");
    std::string line;
    while (std::getline(proc, line))
    {
        if (line.find(INTR_NAME) != std::string::npos)
        {
            if ((line.find("msi") != std::string::npos && line.find("msix") == std::string::npos))
            {
                return InterruptType::MSI;
            }
            else if (line.find("msix") != std::string::npos)
            {
                return InterruptType::MSIX;
            }
        }
    }
    return InterruptType::LEGACY;
}
...
//uaddr[0xa0] = 9;
    ...
    auto pcie_dma_initial = [=]() {
		...
		h2c_fd = open("/dev/pcie_dma0_htc_0", O_RDWR);
        c2h_fd = open("/dev/pcie_dma0_cth_0", O_RDWR);
		...
		off_t reg_offset = 0x3c0;
        uint32_t intr_reg_wr_val;
        int rc;
        switch (check_InterruptType())
        {
        case InterruptType::MSI:
            intr_reg_wr_val = 0x2;
            break;
        case InterruptType::MSIX:
            intr_reg_wr_val = 0x1;
            break;
        case InterruptType::LEGACY:
            intr_reg_wr_val = 0x0;
            break;
        }
        rc = pwrite(nufd, &intr_reg_wr_val, sizeof(intr_reg_wr_val), reg_offset);

		
		
        volatile uint32_t *naddr = (uint32_t *)mmap(0, USER_BAR_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, nufd, 0);
        if(naddr == MAP_FAILED) {
            ui->log_window->append(QString::asprintf("map pcie_dma0_user failed: %s", strerror(errno)));
            ::close(nufd);
            nufd = -1;
            return;
        }
        ui->log_window->append("open pcie_dma0_user successed! BAR_SIZE: " + QString::asprintf("%X", USER_BAR_SIZE));
