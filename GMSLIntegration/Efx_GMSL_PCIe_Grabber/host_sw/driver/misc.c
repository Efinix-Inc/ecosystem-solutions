...
#define HTC_DESC_SRC_ADDR_L_R 0x40008
#define HTC_DESC_SRC_ADDR_H_R 0x4000c
#define CTH_DESC_SRC_ADDR_L_R 0x40010
#define CTH_DESC_SRC_ADDR_H_R 0x40014
#define BYPASS_DIR_HTC 0
#define BYPASS_DIR_CTH 1
...
void misc_bypass_test(struct efx_pci_dev *epdev, int dir)
{
	...
	reg = epdev->bar[0] + HTC_DESC_SRC_ADDR_L_R;
	iowrite32(reg_val, reg);
	reg = epdev->bar[0] + CTH_DESC_SRC_ADDR_L_R;
	iowrite32(reg_val, reg);

	reg_val = (bypass_buf_dma >> 32) & 0xFFFFFFFF;
	pr_info("BYPASS set address High32bit=0x%08x\n", reg_val);
	reg = epdev->bar[0] + HTC_DESC_SRC_ADDR_H_R;
	iowrite32(reg_val, reg);
	reg = epdev->bar[0] + CTH_DESC_SRC_ADDR_H_R;
	iowrite32(reg_val, reg);
	...
	if (dir == BYPASS_DIR_HTC)
	{
		...
		iowrite32(reg_val, reg); // enable HTC running bit & poll mode
		reg_val = 0;
	...
	pr_info("BYPASS %s test done! wb count=%d\n", dir ? "CTH" : "HTC", wb_data->completed_desc_count);
	// reg_val = ioread32(reg);
	return;
}
								  ...
								  const char *buf, size_t count)
{
	...
	{
		misc_bypass_test(epdev, BYPASS_DIR_HTC);
	}
	else if (!strncmp(buf, "cth", 3))
	{
		misc_bypass_test(epdev, BYPASS_DIR_CTH);
	}
	return count;
}
								  ...
								  const char *buf, size_t count)
{
	...
	else
	{
		...
		pr_info("Configuration for MSI-X done!\n");

		// HTC msi configuration
		msi_offset = 0xF00;
		// pci_read_config_dword(epdev->pdev, 0x94, &msi_addr);
		// iowrite32(msi_addr, reg + reg_oft + msi_offset);
		// pci_read_config_dword(epdev->pdev, 0x98, &msi_addr);
		// iowrite32(msi_addr, reg + reg_oft + 0x04 + msi_offset);
		iowrite32(0, reg + reg_oft + 0x08 + msi_offset);
		// iowrite32(0, reg + reg_oft + 0x0C);
		// // CTH msi configuration
		// pci_read_config_dword(epdev->pdev, 0x94, &msi_addr);
		// iowrite32(msi_addr, reg + reg_oft + 0x10 + msi_offset);
		// pci_read_config_dword(epdev->pdev, 0x98, &msi_addr);
		// iowrite32(msi_addr, reg + reg_oft + 0x14 + msi_offset);
		iowrite32(1, reg + reg_oft + 0x18 + msi_offset);
		// iowrite32(0, reg + reg_oft + 0x1C);
