...
# PLL Constraints
#################
create_clock -period 20.000 -name io_peripheralClk    [get_ports {io_peripheralClk}]
create_clock -period 10.000 -name io_memoryClk        [get_ports {io_memoryClk}]
create_clock -period 16.667 -name io_ddrMasters_0_clk [get_ports {io_ddrMasters_0_clk}]
create_clock -period 12.500 -name io_systemClk        [get_ports {io_systemClk}]
create_clock -period 100.00 -name jtag_inst1_TCK      [get_ports {jtag_inst1_TCK}] 

set_clock_groups -exclusive -group {io_systemClk} -group {io_memoryClk} -group {io_peripheralClk} -group {io_ddrMasters_0_clk} -group {jtag_inst1_TCK}
# GPIO Constraints
####################
...
############################
# set_input_delay -clock <CLOCK> [-reference_pin <clkout_pad>] -max <MAX CALCULATION> [get_ports {buttonReset}]
# set_input_delay -clock <CLOCK> [-reference_pin <clkout_pad>] -min <MIN CALCULATION> [get_ports {buttonReset}]
